...
\end{figure}
Durch Angabe der Update-Seite können beide Features ausgewählt werden. Nach zweimaligem Klick auf \textit{Weiter} und anschliessender Bestätigung der Lizenzbestimmungen wird die Software installiert. Danach soll die Entwicklungsumgebung neu gestartet werden.


\section{Update}
Wenn ein Update der Software verfügbar ist, kann via \textit{Hilfe / Nach Updates suchen} das Update-Fenster geöffnet werden. Die  Softwarepakete werden anschliessend heruntergeladen und aktualisiert.
 \begin{figure}[H]
  	\centering
...
\section{Profile}
Zur Verwaltung der verschiedenen Analyseprofile gibt es die View \textit{Profile}. In ihr können Profile erstellt, exportiert und importiert werden. Wenn eine Logdatei mit einem bestimmten Analyseprofil geöffnet werden soll, muss das entsprechende Profil in der Ansicht \textit{Profile} selektiert sein.

\subsection{Profil erstellen}
Mit Rechtsklick auf die View \textit{Profile / Profil erstellen} oder \textit{Datei / Neu / Andere... / Analysis Profile} (Kategorie Garbage Collection) kann der Dialog zum Erstellen eines neuen Profils geöffnet werden. 
 \begin{figure}[H]
  	\centering
    	\includegraphics[width=10cm]{images/tutorial_newprofile}
        	\caption{Profil erstellen}
\end{figure}
Sofern die Erweiterung für die das Profil erstellt wird selektiert und ein Name vergeben ist, wird mittels Klick auf \textit{Finish} das Profil angelegt.
...
Sofern in der Ansicht \textit{Profile} kein Profil selektiert ist, wird mit Doppelklick auf eine importierte Logdatei in der Ansicht \textit{Logdateien} die Standardauswertung geöffnet. Die Standardauswertung besteht aus drei unterschiedlichen Tabs. Der erste Tab zeigt eine Zusammenfassung der Garbage Collection, der zweite Tab den Verlauf des benötigten Speichers auf dem Heap über die Zeit, und der dritte Tab die Dauer der einzelnen Garbage Collection Zyklen über die Zeit. 
